...
variables2HE = ['pho1ecalPFClusterIso','pho1trkSumPtHollowConeDR03','pho1PFsumNeutralHadronEt','pho1SigmaIetaIeta','pho1Smajor','pho1Sminor','pho1R9','pho1SeedTimeRaw', 'pho1HoverE']
...
df_bkg = rp.read_root(bkgFileName,
                          'DelayedPhoton', columns=variables)
df_signalHE = rp.read_root(signalFileName,
                           'DelayedPhoton', columns=variablesHE)
df_bkgHE = rp.read_root(bkgFileName,
                          'DelayedPhoton', columns=variablesHE)
df_signal2 = rp.read_root(signalFileName,
                             'DelayedPhoton', columns=variables2)
df_bkg2 = rp.read_root(bkgFileName,
                          'DelayedPhoton', columns=variables2)
df_signal2HE = rp.read_root(signalFileName,
                             'DelayedPhoton', columns=variables2HE)
df_bkg2HE = rp.read_root(bkgFileName,
                          'DelayedPhoton', columns=variables2HE)
...
xHE = np.concatenate([df_bkgHE.values,df_signalHE.values])
yHE = np.concatenate([np.zeros(len(df_bkgHE)),
                        np.ones(len(df_signalHE))])
x2 = np.concatenate([df_bkg2.values,df_signal2.values])
y2 = np.concatenate([np.zeros(len(df_bkg2)),
                        np.ones(len(df_signal2))])
x2HE = np.concatenate([df_bkg2HE.values,df_signal2HE.values])
y2HE = np.concatenate([np.zeros(len(df_bkg2HE)),
                        np.ones(len(df_signal2HE))])

# split data into train and test sets
seed = 7
test_size = 0.4
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=test_size, random_state=seed)
x_trainHE, x_testHE, y_trainHE, y_testHE = train_test_split(xHE, yHE, test_size=test_size, random_state=seed)
x_train2, x_test2, y_train2, y_test2 = train_test_split(x2, y2, test_size=test_size, random_state=seed)
x_train2HE, x_test2HE, y_train2HE, y_test2HE = train_test_split(x2HE, y2HE, test_size=test_size, random_state=seed)
...
model = XGBClassifier()
modelHE = XGBClassifier()
model2 = XGBClassifier()
model2HE = XGBClassifier()
#model.fit(x_train, y_train, sample_weight=sample_weights)
model.fit(x_train, y_train)
modelHE.fit(x_trainHE, y_trainHE)
model2.fit(x_train2, y_train2)
model2HE.fit(x_train2HE, y_train2HE)
...
y_predHE = modelHE.predict_proba(x_testHE)[:, 1]
predictionsHE = [round(value) for value in y_predHE]
y_pred2 = model2.predict_proba(x_test2)[:, 1]
predictions2 = [round(value) for value in y_pred2]
y_pred2HE = model2HE.predict_proba(x_test2HE)[:, 1]
predictions2HE = [round(value) for value in y_pred2HE]
...
fprHE, tprHE, _ = roc_curve(y_testHE, y_predHE)
fpr2, tpr2, _ = roc_curve(y_test2, y_pred2)
fpr2HE, tpr2HE, _ = roc_curve(y_test2HE, y_pred2HE)
...
plt.plot(fprHE, tprHE, color='red',
         lw=lw, label='7 variables and H/E')
plt.plot(fpr2, tpr2, color='blue',
         lw=lw, label='7 variables and ECAL timing')
plt.plot(fpr2HE, tpr2HE, color='green',
         lw=lw, label='7 variables, ECAL timing, and H/E')
...
# Plot
plt.figure()
plt.bar(range(len(model2HE.feature_importances_)), model2HE.feature_importances_)
#plt.show()
